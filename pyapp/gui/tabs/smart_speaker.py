...
class SmartSpeakerTab(ttk.Frame):
    def __init__(self, parent, session, root: tk.Tk):
        ...
        self.display_asset_var = tk.StringVar(value="")
        self.display_is_anim_var = tk.BooleanVar(value=False)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(testf, text="Картинка/папка анимации").grid(row=2, column=0, sticky=tk.W, padx=4, pady=2)
        ttk.Entry(testf, textvariable=self.display_asset_var, width=40).grid(row=2, column=1, columnspan=2, sticky=tk.EW, padx=4, pady=2)
        ttk.Button(testf, text="Обзор…", command=self._browse_display).grid(row=2, column=3, padx=4, pady=2)
        ttk.Checkbutton(testf, text="Режим анимации (папка с кадрами)", variable=self.display_is_anim_var).grid(
            row=3, column=0, columnspan=2, sticky=tk.W, padx=4, pady=2
        )
        ttk.Label(testf, text="Текст ответа").grid(row=4, column=0, sticky=tk.NW, padx=4, pady=2)
        self.text_w = scrolledtext.ScrolledText(testf, width=52, height=6, wrap=tk.WORD)
        self.text_w.grid(row=4, column=1, columnspan=3, sticky=tk.EW, padx=4, pady=2)
        bf = ttk.Frame(testf)
        bf.grid(row=5, column=0, columnspan=4, sticky=tk.W, padx=4, pady=4)
    ...
    def _browse_display(self) -> None:
        p = filedialog.askopenfilename(
            title="Картинка для дисплея",
            filetypes=[("Изображения", "*.jpg *.jpeg *.png *.bmp"), ("Все файлы", "*.*")],
        )
        if p:
            self.display_asset_var.set(p)
    ...
    def _simulate_response(self) -> None:
        ...
        disp_path = self.display_asset_var.get().strip()
        payload = ResponsePayload(
            audio_path=Path(audio_path) if audio_path else None,
            text=text,
            display_asset=Path(disp_path) if disp_path else None,
            is_animation=self.display_is_anim_var.get(),
        )
