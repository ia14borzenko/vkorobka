...
### 6.1) Проверка встроенной справки CLI (`--help`)

Из каталога репозитория:

```powershell
python pyapp\main.py --help
python pyapp\show_bg.py --help
python pyapp\texting_cli.py --help
python pyapp\voice_cli.py --help
python pyapp\speaker_cli.py --help
```

Что покрывает каждая CLI-утилита:
- `main.py` — базовый интеграционный тест зеркальной обработки JPG для `win` и `esp32`.
- `show_bg.py` — отправка RGB565-кадра на LCD ESP32 с настройкой `chunk_rows`.
- `texting_cli.py` — команды `clear` / `add_text`, параметры текстового поля и регенерация шрифтового кэша.
- `voice_cli.py` — интерактивная запись `voice.on` / `voice.off`, сохранение `FLAC/WAV/TSV`.
- `speaker_cli.py` — воспроизведение файла на динамике ESP32 (`dyn.set`/`dyn.on`/`dyn.off`).

### 6.2) Проверка справки GUI по вкладкам

В `pyapp/gui_app.py` встроена актуальная справка:
- Откройте GUI: `python pyapp\gui_app.py`
- Нажмите **ПКМ** в окне -> **«Справка по вкладкам»**
- Там описаны вкладки `Сеть`, `Дисплей`, `Текстинг`, `Микрофон`, `Динамик`, `Умная колонка`, `Диагностика`, а также контекстное меню и журнал.

Отдельно по GUI:
- Общие аудиопараметры (`mic/dyn`) задаются в левой панели и используются несколькими вкладками.
- Для анализа входящего аудио доступен пункт **«График микрофона»** в контекстном меню.
