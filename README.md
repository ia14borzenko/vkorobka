...
- **mic_stream** (`mic_stream.hpp/cpp`) — I2S-приём с INMP441, **`voice.on` / `voice.off`**, uplink PCM (`stream_id = 2`).
- **dyn_playback** (`dyn_playback.hpp/cpp`) — I2S на MAX98357A, **`dyn.on` / `dyn.off`**, приём PCM по TCP (`stream_id = 3`).
...
|------|-------------------------|--------------------------|----------------------|------------------------|
| **Микрофон** (INMP441) | `voice.on` / `voice.off` | `stream_id = 2` | **48 kHz**, **24-bit LE**, до **512** сэмплов/пакет | ESP32 → EXTERNAL (pyapp) |
| **Динамик** (MAX98357A) | `dyn.on` / `dyn.off` | `stream_id = 3` | **22050 Hz**, **16-bit LE**, до **512** сэмплов/пакет | EXTERNAL → ESP32 |

Команды: тип **`MSG_TYPE_COMMAND`**, источник **`EXTERNAL`**, назначение **`ESP32`**, полезная нагрузка — **текстовая строка** в байтах (в JSON от pyapp она уходит в **base64**, как и остальные команды). Ответы ESP32: тип **`MSG_TYPE_RESPONSE`**, назначение **`EXTERNAL`**, текст **`VOICE_ON_OK`**, **`VOICE_OFF_OK`**, **`DYN_ON_OK`**, **`DYN_OFF_OK`**.
...
| Поле | Тип | Описание |
|------|-----|----------|
| `sample_rate_hz` | `u32` | 48000 (микрофон) или 22050 (динамик) |
...
- **I2S:** приём **Philips**, **стерео 32-bit** слоты, **48 kHz**; с шины берётся **левый** канал (вывод L/R модуля INMP441 на GND — см. таблицу пинов ниже).
- **Контроллер:** в прошивке используется **I2S0** (отдельно от динамика).
- **Усиление:** перед упаковкой в сеть к отсчётам применяется **+3 dB** по амплитуде (коэффициент ≈ 10^(3/20), с ограничением по диапазону int24) — см. `mic_stream.cpp`.
...
Команды в консоли: **`voice.on`** (начать накопление потока), **`voice.off`** (остановить и сохранить FLAC/WAV/TSV), **`quit`**. Нужны `numpy`, `soundfile`.

### Динамик (детали)

- **I2S:** передача **Philips**, тактовая частота **22050 Hz** (ниже, чем у микрофона, чтобы разгрузить сеть); **32-bit** стерео-слоты, в L и R подаётся один и тот же моно-сэмпл (**16-bit** → старшие 16 бит слота).
...
Файл читается целиком, при необходимости **ресэмплинг в 22050 Hz** (scipy), затем `dyn.on` → чанки STREAM (в JSON чанки идут **без поля `test_id`**, чтобы на win-x64 не затирать привязку ответов между `dyn.on` и `dyn.off`) → пауза **~0.35 с** → `dyn.off`. Параметры: `--chunk-samples` (1…512), `--no-pace`, `--pace-factor` (по умолчанию около реального времени). Константы формата в коде: `VkorobkaClient.DYN_PCM_SAMPLE_RATE_HZ`, `DYN_PCM_BITS` и совпадающие константы в `dyn_playback.cpp`.
